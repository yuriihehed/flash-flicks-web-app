...
(function() {
  ...
  function markLearned(button, learned) {
    const flashcardId = button.getAttribute("data-flashcard-id");

    console.log(`📌 Button clicked: Flashcard ID = ${flashcardId}, Learned = ${learned}`);
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            const statusElement = document.querySelector(`.term-box[data-flashcard-id="${flashcardId}"] .learned-status`);
            if (statusElement) {
